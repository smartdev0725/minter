...
abstract contract FeePayer is AdministrateeInterface, Testable, Lockable {
  ...
  function _gulp() internal {
    ...
    FixedPoint.Unsigned memory currentBalance =
      FixedPoint.Unsigned(collateralCurrency.balanceOf(address(this)));
  ...
  function _getPendingRegularFeeAdjustedCollateral(
    ...
    if (
      currentTotalOutstandingRegularFees.isEqual(FixedPoint.fromUnscaledUint(0))
    ) return rawCollateral;

    // Calculate the total outstanding regular fee as a fraction of the total contract PFC.
    FixedPoint.Unsigned memory effectiveOutstandingFee =
      currentTotalOutstandingRegularFees.divCeil(_pfc());
  ...
  function _removeCollateral(
    ...
    FixedPoint.Unsigned memory adjustedCollateral =
      _convertToRawCollateral(collateralToRemove);
  ...
  function _addCollateral(
    ...
    FixedPoint.Unsigned memory initialBalance =
      _getFeeAdjustedCollateral(rawCollateral);
    FixedPoint.Unsigned memory adjustedCollateral =
      _convertToRawCollateral(collateralToAdd);
