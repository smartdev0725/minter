...
abstract contract FundingRateApplier is EmergencyShutdownable, FeePayer {
      ...
      returns (int256 price) {
        ...
        if (proposalTime >= lastUpdateTime) {
          ...
          if (request.disputer == address(0)) {
            reward = _pfc().mul(_getConfig().rewardRatePerSecond).mul(
              proposalTime.sub(lastUpdateTime)
            );
          ...
          emit FundingRateUpdated(
            fundingRate.rate.rawValue,
            fundingRate.updateTime,
            reward.rawValue
          );
        }
    ...
    returns (FixedPoint.Unsigned memory newCumulativeFundingRateMultiplier)
  {
    ...
    FixedPoint.Signed memory periodRate =
      fundingRatePerSecond.mul(SafeCast.toInt256(paymentPeriodSeconds));
    ...
    newCumulativeFundingRateMultiplier = currentCumulativeFundingRateMultiplier
      .mul(unsignedPeriodMultiplier);
  }
